...
This is where the concept of **conditional probability** becomes relavent. Given a sample space $\Omega$ 
...
Said differently, the second draw computes the the probability of observing an $8$ _conditioned upon_ our 
...
A prosecutor at court presents evidence $\mathcal{E}$. They argue that the defendent is guilty because the probability
of finding the evidence given innocence $P(\mathcal{E}|\mathcal{I})$ is small. But this is totally irrelevant. The real
question is what is the probability that the defendent is innocent given the evidence: $P(\mathcal{I}|\mathcal{E})$. 
...
To work this out, we need to know $P(G_3|C_1)$, $P(G_3|C_2)$, and $P(G_3|C_3)$. 

> + $P(G_3|C_3)=0$, because the host chooses from doors with goats behind them. 
> + $P(G_3|C_1)=\frac{1}{2}$, because the host chooses randomly from doors with goats behind them that are not door one. 
> + $P(G_3|C_2)=1$, because there is only one door that (a) has a goat behind it and (b) isn’t door one. 
...
There are many cases in Astronomy and Physics where anomoly detection is interesting/desireable. Discovering new and
...
Let $\mathcal{d}$ be the event that a true decay occurs, and $\mathcal{P}$ be the event that the experiement produces a 
positive detection. 
...
Let's now consider a similar scenario, except the probability of our decay occuring in nature is very small:
