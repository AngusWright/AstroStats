...
> + The PDF is non-negative, because the $P(u)<0$ would require $$ \int_{u-\epsilon}^{u+\epsilon} p(x)\mathrm{d}x<0 $$, which is impossible. 
...
```{r,  fig.height=4, fig.width=6, out.width='90%',eval=TRUE,echo=FALSE}
set.seed(10)
magcurve(qgamma(x,shape=1.2,scale=20),from=0.01,to=0.99,,ylab='Delay (mins)',
         xlab=expression(P(X<=x)),
...
> + the probability that you find a point doesn’t depend on where you are in the 
domain. 
...
is defined by two parameters, the shape parameter $\alpha$ and the inverse scale parameter $\beta=1/s$,
also called the rate parameter.  The gamma distribution PDF is defined as: 
$$
\begin{aligned}
...
The variability of the Gamma distribution as a function of shape parameter $\alpha$ and rate
parameter $\beta$ are shown below: 

```{r,  fig.height=4, fig.width=6, out.width='90%',eval=TRUE,echo=FALSE}
magcurve(dgamma(x,1,1),from=0,to=10,col=seqinr::col2alpha('red2',0.5),lwd=4,
        ylim=c(0,2),xlab='x',ylab='PDF',
        main="Gamma Distribution with identical shape and rate params")
...
legend('topright',legend=c(expression(paste("X~Ga(",alpha,"=1,",beta,"=1)")),expression(paste("X~Ga(",alpha,"=5,",beta,"=5)")),
                           expression(paste("X~Ga(",alpha,"=15,",beta,"=15)"))),lty=1,
       col=c('red2','green3','blue2'),lwd=2,inset=0.1,bty='n')
```

```{r,  fig.height=4, fig.width=6, out.width='90%',eval=TRUE,echo=FALSE}
magcurve(dgamma(x,1,5),from=0,to=10,col=seqinr::col2alpha('red2',0.5),lwd=4,
        ylim=c(0,2),xlab='x',ylab='PDF',main="Gamma Distribution with different shape and rate params")
...
legend('topright',legend=c(expression(paste("X~Ga(",alpha,"=1,",beta,"=5)")),expression(paste("X~Ga(",alpha,"=5,",beta,"=1)")),
                           expression(paste("X~Ga(",alpha,"=15,",beta,"=3)")),expression(paste("X~Ga(",alpha,"=20,",beta,"=10)"))),lty=1,
       col=c('red2','green3','blue2','orange2'),lwd=2,inset=0.1,bty='n')
```
